...
function showPrompt(){
  topPrompt.classList.remove('show');
  setTimeout(()=>{
    const title=currentMusicTitle(),showTitle=Boolean(title&&nextIsMusicTitle),content=document.createElement('span');
    content.className=showTitle?'prompt-content track-title':'prompt-content';
    if(showTitle){content.textContent=title;nextIsMusicTitle=false}else{content.innerHTML=topPrompts[promptIndex];promptIndex=(promptIndex+1)%topPrompts.length;if(title)nextIsMusicTitle=true}
    topPrompt.replaceChildren(content);topPrompt.classList.toggle('music-mode',showTitle);topPrompt.classList.add('show');
  },600)
}
...
const controls=$('.controls'),musicDevice=$('#musicDevice'),micDevice=$('#micDevice');function toggleControls(force){const open=force??!controls.classList.contains('open');controls.classList.toggle('open',open);controls.setAttribute('aria-hidden',String(!open))}$('#closeControls').onclick=()=>toggleControls(false);
function connected(id,on=true){$(id).classList.toggle('connected',on)}
async function loadDevices(){try{status.textContent='SE CAUTĂ SURSELE AUDIO';const devices=await audio.listDevices();for(const select of [musicDevice,micDevice]){select.innerHTML='';devices.forEach((d,i)=>{const option=document.createElement('option');option.value=d.deviceId;option.textContent=d.label||`Dispozitiv audio ${i+1}`;select.appendChild(option)})}const cable=[...musicDevice.options].find(o=>/CABLE Output|VB-Audio|Virtual/i.test(o.textContent));const physical=[...micDevice.options].find(o=>!/CABLE|VB-Audio|Virtual/i.test(o.textContent));if(cable)musicDevice.value=cable.value;if(physical)micDevice.value=physical.value;status.textContent=cable?'SURSE GĂSITE · CONECTEAZĂ AMBELE':'CABLE LIPSEȘTE · FOLOSEȘTE AUDIO SISTEM'}catch(err){status.textContent=err?.message==='AUDIO_REQUIRES_HTTP'?'DESCHIDE PRIN LINKUL HTTP':'ACCES AUDIO REFUZAT'}}
...
function loop(t){
  ...
  if(t-lastRenderedFrame<FRAME_INTERVAL)return;
  lastRenderedFrame=t-(t-lastRenderedFrame)%FRAME_INTERVAL;
...
toggleControls(true);status.textContent=location.protocol==='file:'?'DESCHIDE PRIN PORNESTE-LIVE-CURAT':'1. REÎNCARCĂ SURSE · 2. CONECTEAZĂ MUZICA · 3. CONECTEAZĂ MICROFONUL';

window.addEventListener('djsoul:youtubeplaylistready',event=>{
  const state=event.detail||{},first=state.items?.[0];
  if(first&&!manualTitle){musicTitleData.textContent=first.title||'';musicTitleData.dispatchEvent(new CustomEvent('live-title-change',{detail:{playing:true,source:'youtube'}}))}
});
